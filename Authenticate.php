<?php
	...
	if(!$result) { // If it doesn't exist
		...
		if(strcmp($passwordHash, $result[UserConnector::$COLUMN_PASSWORDHASH]) == 0) { // If password matches
			...
			$voted_comments = $CommentVotesConnector->selectByUser($result[UserConnector::$COLUMN_ID]);
			$response["voted_comments"] = array();
			for($i = 0; $i < count($voted_comments); $i++) {
				array_push($response['voted_comments'], $voted_comments[$i]['comment_id']);
			}
 		}
